...
def backward():
    ...
    learning_rate = tf.train.exponential_decay(
        LEARN_RATE_BASE,
        global_step,
        decay_steps=NUM_TRAIN_EXAMPLE/BATCH_SIZE, #have changed
    ...
    img_batch,label_batch= dataset_make.get_tfrecord(BATCH_SIZE, True)
    #不需要在会话中进行?
    with tf.Session() as sess:
        ...
        coord = tf.train.Coordinator() #have changed
        threads = tf.train.start_queue_runners(sess=sess, coord=coord) #have changed
        ...
        coord.request_stop()  #have changed
        coord.join(threads)  #have changed
